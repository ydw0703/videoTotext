...
# =================== 동영상 테스트 ================================
    ...
    for i, frame in enumerate(frame_images):
        vp.save_image(frame,'C:\\Users\\Administrator\\Desktop\\2021_ICT\\test2\\frame_{}.jpg'.format(i))
        final_result=[]
        copy = ct.resize(frame)
        cropped_images=ct.image_all_process(copy)
        # 섹션 추출 
        ct.save_crooped_contours(cropped_images[0]["section"], 'C:\\Users\\Administrator\\Desktop\\2021_ICT\\test2\\section_{}'.format(i))
        
        for con in cropped_images[0]["contours"]:
            # 프레임에서 추출한 영역들 저장 (origin 로)
            ct.save_crooped_contours(con, 'C:\\Users\\Administrator\\Desktop\\2021_ICT\\test2\\contours_{}_frame_{}_'.format(num2, i))
            num2 += 1
        
        # 한 프레임의 글자 영역들에서 텍스트 추출 과정 시작 
        for con in cropped_images[1]["contours"]:
            # 프레임에서 추출한 영역들 저장 (gray-scale로)
            ct.save_crooped_contours(ct.get_gradient(con), 'C:\\Users\\Administrator\\Desktop\\2021_ICT\\test2\\contours_{}_frame_{}_'.format(num,i))
            num += 1
    ...
    for j in range(1, num):
        ...
        if (second in section.keys()):
            ...
            section[second] = vision.image_ocr('C:\\Users\\Administrator\\Desktop\\2021_ICT\\test2\\section_{}.jpg'.format(second))

        # 텍스트인 contour 찾아내기 
        if judge_result[j] == 'text':
            val = vision.image_ocr('C:\\Users\\Administrator\\Desktop\\2021_ICT\\test2\\contours/'+flie_list[j])
 ...
 # 모든 결과 값들 저장 하기
    txt.directory_save(tmp_result, section, 'C:\\Users\\Administrator\\Desktop\\2021_ICT\\test2\\jjan.csv')
